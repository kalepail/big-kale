...
const cam = { x: 18 * TILE, y: 14 * TILE, z: 1.15 };
const keys = new Set();
let tool = "plant";
let drag = null;
let selected = null;
...
function connect() {
  ...
  ws.onopen = () => toast("connected to kale-1. one planter. forty kale. write a file.");
  ws.onclose = () => { toast("world napped. reconnecting…"); setTimeout(connect, 1200); };
  ws.onerror = () => {};
  ws.onmessage = (ev) => {
    let data;
    try { data = JSON.parse(ev.data); } catch { return; }
    if (!data || typeof data !== "object") return;
    if (data.type === "cf_agent_state" && data.state?.farm) return;
    if (data.type === "map") map = data;
    if (data.type === "snap") {
      snap = data;
      paintHud();
      if (selected?.kind === "agent") {
        const a = data.agents.find((x) => x.id === selected.id);
        if (a) renderInspectAgent(a);
      }
    }
    if (data.type === "ack" && data.msg) toast(data.msg);
  };
}
connect();
